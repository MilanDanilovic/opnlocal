<script lang="ts">
  ...
  // Every license text of the bundled components (generated by scripts/generate-notices.mjs).
  let fullNotices = $state<string | null>(null);
  async function loadNotices() {
    fullNotices ??= await fetch("/THIRD_PARTY_NOTICES.txt")
      .then((r) => (r.ok ? r.text() : t.errors.generic))
      .catch(() => t.errors.generic);
  }
    ...
    <section class="card stack" aria-labelledby="about">
      ...
      <details>
        ...
        <details ontoggle={loadNotices}>
          <summary>{t.settings.fullNotices}</summary>
          <!-- svelte-ignore a11y_no_noninteractive_tabindex -->
          <pre class="report tall" tabindex="0" aria-label={t.settings.fullNotices}>{fullNotices ?? "…"}</pre>
        </details>
      </details>
    </section>
  </div>
